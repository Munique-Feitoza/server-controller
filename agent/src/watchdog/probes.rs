...
// Compatível com: nginx, nginx-rc, mysql, postgresql, php-fpm, gunicorn, docker
// ─────────────────────────────────────────────────────────────────────────────

/// Configuração de uma probe de serviço systemd
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ServiceProbeConfig {
    /// Nome do daemon (ex: "nginx-rc", "mysql", "postgresql", "php8.1-fpm")
...
pub fn default_probes_for_role(role: &str) -> Vec<AnyProbeConfig> {
    match role {
        "wordpress" => vec![
            AnyProbeConfig::Service(ServiceProbeConfig { service_name: "nginx-rc".to_string() }),
            AnyProbeConfig::Service(ServiceProbeConfig { service_name: "apache2-rc".to_string() }),
            AnyProbeConfig::Service(ServiceProbeConfig { service_name: "mariadb".to_string() }),
            AnyProbeConfig::Service(ServiceProbeConfig { service_name: "php81rc-fpm".to_string() }),
        ...
        // "generic" — detecta automaticamente se eh RunCloud ou padrao
        _ => {
            // Verifica se nginx-rc existe (RunCloud)
            let nginx_name = if std::process::Command::new("systemctl")
                .args(["is-active", "nginx-rc"])
                .output()
