...
impl ThreatTracker {
    ...
    fn ip_seguro(ip: &IpAddr) -> bool {
        ...
        SAFE_PREFIXES.iter().any(|prefix| ip_str.starts_with(prefix))
    }
    ...
    pub fn registrar_honeypot(&mut self, ip: IpAddr, path: &str, user_agent: &str) -> ThreatAction {
        ...
        if Self::ip_seguro(&ip) {
            tracing::info!("🔒 IP seguro {} acessou honeypot {} — ignorado (dev/tunel SSH)", ip, path);
        ...
        if record.count >= BAN_THRESHOLD {
            ...
            self.banned.push(BanRecord {
                ip: ip.to_string(),
                reason: format!("{} acessos a honeypot paths em {}s", record.count, record.first_seen.elapsed().as_secs()),
            ...
            tokio::spawn(async move {
                ...
                if intel.likely_human {
                    tracing::error!(
                        ...
                        intel.ip, intel.city, intel.country, intel.isp, intel.classification_reason
                    );
                } else {
                    tracing::warn!(
    ...
    fn limpar_expirados(&mut self) {
        ...
        self.attempts.retain(|_, record| {
            now.duration_since(record.first_seen).as_secs() < WINDOW_SECS
        });
    }
}
...
async fn banir_ip_iptables(ip: &str) {
    ...
    match result {
        ...
        Ok(status) => {
            tracing::error!("❌ iptables retornou codigo {}: IP {} nao bloqueado", status, ip);
        }
        Err(e) => {
...
pub const HONEYPOT_PATHS: &[&str] = &[
    ...
    "/shell", "/cmd", "/webshell", "/c99",
    "/id_rsa", "/config.php", "/wp-config.php",
    "/.htaccess", "/.htpasswd",
