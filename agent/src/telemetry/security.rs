...
impl SecurityMetrics {
    ...
    fn parse_failed_logins() -> Option<Vec<FailedLogin>> {
        ...
        for line in stdout.lines() {
            ...
            if parts.len() >= 3 {
                ...
                let entry = ip_counts
                    .entry(ip)
                    .or_insert((0, parts.get(4..8).map(|s| s.join(" ")).unwrap_or_default()));
                entry.0 += 1;
            }
        }
        ...
        let mut result: Vec<FailedLogin> = ip_counts
            .into_iter()
            .filter(|(_, (count, _))| {
                *count >= crate::telemetry::AlertThresholds::default().security_threat_threshold
            }) // Padrão OMNI-DEV: Foca apenas em ameaças reais
            .map(|(ip, (count, last_attempt))| FailedLogin {
                ip,
                count,
                last_attempt,
            })
    ...
    pub fn block_ip(ip: &str) -> Result<bool> {
        ...
        ip.parse::<std::net::IpAddr>().map_err(|_| {
            crate::error::AgentError::CommandError(format!(
                "Endereço IP inválido: '{}'. Apenas IPs individuais são aceitos (ex: 192.168.1.1)",
                ip
            ))
        })?;
        ...
        let status = Command::new("iptables")
            ...
            .status()
            .map_err(|e| {
                crate::error::AgentError::CommandError(format!("Iptables failed: {}", e))
            })?;
