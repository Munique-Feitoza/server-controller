...
deploy_to() {
    ...
    # 1. Upload do binário
    scp -i "$KEY" -P "$PORT" -o StrictHostKeyChecking=no "$BIN_PATH" "deploy@$IP:/home/deploy/pocket-noc-agent.tmp"
    
    # 2. Substituição e Restart (Usa sudo remoto para o systemctl)
    ssh -i "$KEY" -p "$PORT" -o StrictHostKeyChecking=no "deploy@$IP" "
        sudo systemctl stop pocket-noc-agent
        sudo mv /home/deploy/pocket-noc-agent.tmp /usr/local/bin/pocket-noc-agent
